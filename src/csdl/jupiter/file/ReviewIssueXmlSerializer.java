...
import csdl.jupiter.ReviewException;
import csdl.jupiter.model.review.ReviewId;
import csdl.jupiter.model.review.ReviewModel;
import csdl.jupiter.model.reviewissue.Resolution;
import csdl.jupiter.model.reviewissue.ResolutionKeyManager;
import csdl.jupiter.model.reviewissue.ReviewIssue;
import csdl.jupiter.model.reviewissue.ReviewIssueModel;
import csdl.jupiter.model.reviewissue.Severity;
import csdl.jupiter.model.reviewissue.SeverityKeyManager;
import csdl.jupiter.model.reviewissue.Status;
import csdl.jupiter.model.reviewissue.StatusKeyManager;
import csdl.jupiter.model.reviewissue.Type;
import csdl.jupiter.model.reviewissue.TypeKeyManager;
 ...
 * @version $Id: ReviewIssueXmlSerializer.java,v 1.9 2005/03/28 14:39:52 takuyay Exp $
 */
public class ReviewIssueXmlSerializer {
  ...
  /** The attribute id. */
  private static final String REVIEW_ATTRIBUTE_ID = "id";
  /** The REVIEW_ISSUE element name. */
  private static final String REVIEW_ISSUE = "ReviewIssue";
  /** The REVIEW_ISSUE_ATTRIBUTE_ID attribute name. */
  private static final String REVIEW_ISSUE_ATTRIBUTE_ID = "id";
  /** The REVIEW_ISSUE_META element name. */
  private static final String REVIEW_ISSUE_META = "ReviewIssueMeta";
   ...
   */
  private static Element createReviewIssueElement(ReviewIssue reviewIssue) {
    if (reviewIssue == null) {
      throw new NullPointerException("ReviewIssue instance is null.");
    }
    Element codeReviewRoot = new Element(REVIEW_ISSUE);
    codeReviewRoot.setAttribute(REVIEW_ISSUE_ATTRIBUTE_ID, reviewIssue.getIssueId());
    Element codeReviewMetaElement = new Element(REVIEW_ISSUE_META);
    ...
    Element reviewerElement = new Element(REVIEWER_ID);
   ...
   */
  private static ReviewIssue createReviewIssue(Element codeReviewElement, IFile reviewIFile)
    throws ReviewException {
    if (codeReviewElement == null) {
      new ReviewException("element instance is null");
    }
    try {
      Element codeReviewMetaElement = codeReviewElement.getChild(REVIEW_ISSUE_META);
      ...
      Element reviewerElement = codeReviewElement.getChild(REVIEWER_ID);
      ...
      String id = getAttributeText(codeReviewElement, REVIEW_ISSUE_ATTRIBUTE_ID);
      ...
      return new ReviewIssue(creationDate, lastModificationDate, reviewer, assignedTo,
                             targetFilePath, lineNumber, type, severity, summary, description,
                             annotation, revision, resolution, status, reviewIFile);
    }
  ...
  public static void write(ReviewId reviewId, ReviewIssueModel model, File xmlFile)
    ...
    write(xmlFile, reviewRootElement);
  }
  ...
  public static void writeEmptyCodeReview(File xmlFile) throws ReviewException {
    ...
    Element codeReviewsRootElement = new Element(REVIEW);   
    ReviewModel reviewModel = ReviewModel.getInstance();
    ReviewId reviewId = reviewModel.getReviewIdManager().getReviewId();   
    codeReviewsRootElement.setAttribute(REVIEW_ATTRIBUTE_ID, reviewId.getReviewId());
    try {
    ...
    for (int i = 0; i < iFiles.length; i++) {
      ...
      if (reviewIdName != null && reviewId != null && reviewIdName.equals(reviewId.getReviewId())) {
        ...
        for (Iterator iterator = codeReviewList.iterator(); iterator.hasNext();) {
          ...
          try {
            ReviewIssue reviewIssue = createReviewIssue(codeReviewElement, iFiles[i]);
